...
close all;
Lin=50;
B={};dim=4:1:8;sample=1000000;clnumber=5000;
N=sample/clnumber; stat=rand(1,Lin);stat=stat/sum(stat);

for kappa=1:length(dim)
    n=dim(kappa);Y=zeros(n^2,clnumber);thet={};
    LL=zeros(1,n^2);
    %v=zeros(4,n);
    ...
    for i2=1:Lin
        ...
        %v=ones(1,n)+0.09*rand(1,n);v=v/sum(v);
    ...
    for r=1:length(U)
        ...
        for k1=1:repe
            ...
            T_ls(k1)=sum(abs(eig(Dlast-Dln_LS)))/2;
            T_LMMSE(k1)=sum(abs(eig(Dlast-Dln)))/2;
        end
    end
    co_ls(kappa)=mean(T_ls);
    co_lmmse(kappa)=mean(T_LMMSE);
end
% thet_check=zeros(1,n^2-1);
%  for i=1:n^2-1
%      thet_check(i)=sum(diag(WDN*sigma_operator(:,:,i)))*sqrt(n/(2*(n-1)));
% end
plot(dim,co_ls,'-ok','LineWidth',1.0,'MarkerSize',10);
hold on;
plot(dim,co_lmmse,'-*k','LineWidth',1.0,'MarkerSize',10);
hold off;
xlabel('Dimension of Hilbert space(d)','fontweight','bold','fontsize',10);
ylabel('Sample average trace distance','fontweight','bold','fontsize',10);
legend('LS based estimate','LMMSE based estimate');
